...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')) {
  ...
  $price_e = isset($_POST["price_e"]) ? limpiarCadena($_POST["price_e"]) : "";
  ...
  $namelink = $name;
  $namelink = strtr($namelink, " ", "_");
  $namelink = strtolower($namelink);
  $link = "  https://luisxv.online/producto/" . $namelink;
  switch ($_GET["op"]) {
    case 'store':
      include './imageProduct.php';
      if (empty($id)) {
        $rspta = $products->store($category_id, $ref, $name, $brand_id, $price, $discount, $description, $specs, $embed, $image, $image2, $image3, $price_e, $link);
        echo $rspta ? "El producto ha sido registrrado" : "El producto no se pudo registrar";
      } else {
        $rspta = $products->update($id, $category_id, $ref, $name, $brand_id, $price, $discount, $description, $specs, $embed, $image, $image2, $image3, $link, $price_e);
    ...
    case 'index':
      ...
      while ($reg = $rspta->fetch_object()) {
        ...
        $price_e = number_format($reg->price_e, 0, '', '.');
        $data[] = array(
          "0" =>
          ...
          "8" => $reg->subcategorie,
          "9" => ($reg->image) ?
            "<img src='../files/products/" . $reg->image . "' height='50px' width='50px' >" :
